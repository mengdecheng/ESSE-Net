...
from indecoder import UnetPlusPlusDecoder
# from indecoderCBAM import UnetPlusPlusDecoder
# from indecoderCCModule import UnetPlusPlusDecoder
# from indecoderSimAM import UnetPlusPlusDecoder
# from indecodermssekernelsizes import UnetPlusPlusDecoder
...
class UnetPlusPlus(SegmentationModel):
    ...
    def __init__(
        ...
        encoder_depth: int = 5,
        encoder_weights: Optional[str] = "imagenet",#记得修改这个是与训练权重
...
#用于模型中间过程返回特征图
    ...
    #
    # def forward(self, x):
    #     """Sequentially pass `x` trough model`s encoder, decoder and segmentation head"""
    #
    ...
    #
    #     # 修改这里来接收两个返回值
    #     # decoder_output, intermediate_outputs = self.decoder(*features)
    #
    ...
    #         return masks, labels
    #
    #     # 如果 visualize 标志为 True，则返回 masks 和中间输出
    #     if hasattr(self, 'visualize') and self.visualize:
    #         return masks, intermediate_outputs
    #
    #     return masks